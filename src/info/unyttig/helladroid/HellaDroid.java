...
/*
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences preference = getSharedPreferences("HellaDroid", 0);
		Preferences.update(preference);

		hnzb = new HellaNzb("http://10.0.2.2", "8760", "90580570");
		dSpeed = (TextView) findViewById(R.id.dSpeed);
		hNzbText = (TextView) findViewById(R.id.hnzbText);
		hNzbText.setOnCreateContextMenuListener(this);
		// dSpeed.setText("1150kb/s");
		ToggleButton togglePause = (ToggleButton) findViewById(R.id.togglePaused);
				...
				try {
					if (hnzb.isAlive()) {
						if (((ToggleButton) v).isChecked()) {
							...
							DisplayToast("Unpaused");
							hnzb.setPaused(false);
						}
					} else DisplayToast("HellaNZB is dead");
				} catch (XMLRPCException e) {
					DisplayToast("HellaNZB is dead");
				}
			}
		});
	}
	...
	public void onCreateContextMenu(ContextMenu menu, View view,
			ContextMenuInfo menuInfo) {
		super.onCreateContextMenu(menu, view, menuInfo);
		createHellaMenu(menu);
	}

	@Override
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		switch (item.getItemId()) {
		case R.id.menuPreferences: startActivity(new Intent(this, Preferences.class)); return true;
		case R.id.menuQuit:
			...
			// this.finish();
			return true;
		case R.id.menuTestServer:
			if (hnzb.isAlive())
				DisplayToast("HellaNZB lives!");
			else
				DisplayToast("HellaNZB is dead");
			return true;
		}
	...
	private void createHellaMenu(Menu menu) {
		MenuInflater inflater = getMenuInflater();
		inflater.inflate(R.menu.hellanzb_menu, menu);
	}

	private boolean hellaDown() {
		if (hnzb.isAlive()) {
			alertDialog = new AlertDialog.Builder(this).create();
			alertDialog.setTitle("HellaNZB");
			alertDialog.setMessage("Really shutdown HellaNZB server?");
			...
			DisplayToast("Server already dead");
		}
		return true;
	}*/